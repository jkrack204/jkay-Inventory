...
(function () {
  ...
  async function renderLocation(el, locId) {
    ...
    function renderDcRows() {
      ...
      dcListEl.innerHTML = rows.map((dc) => {
        ...
        return `
          <button type="button" class="activity-row" data-id="${dc.id}">
            <div class="ar-dc">
      ...
      dcListEl.querySelectorAll('.activity-row').forEach((row) => { row.onclick = () => window.open(`/dc.html?id=${row.dataset.id}`, '_blank'); });
    }
    renderDcRows();
  ...
  async function renderDcs(el) {
    ...
    async function load() {
      ...
      listEl.querySelectorAll('.activity-row').forEach((row) => { row.onclick = () => window.open(`/dc.html?id=${row.dataset.id}`, '_blank'); });
    }
  ...
  async function renderEditLog(el) {
    ...
    async function load() {
      ...
      listEl.innerHTML = edits.map((e) => `
        <button type="button" class="dc-history-row" data-id="${e.dc_id}" style="display:block; width:100%; text-align:left; background:none; border:none; border-bottom:1px solid var(--rule-3); cursor:pointer; padding:12px 4px;">
      ...
      `).join('');
      listEl.querySelectorAll('[data-id]').forEach((row) => { row.onclick = () => window.open(`/dc.html?id=${row.dataset.id}`, '_blank'); });
    }
